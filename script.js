...
function createRadarChart(canvas) {
  ...
  function draw(p) {
    ...
    grad.addColorStop(0, 'rgba(203,41,87,0.2)')
    grad.addColorStop(1, 'rgba(203,41,87,0.04)')
    ctx.fillStyle = grad
    ctx.fill()
...
function createSparkline(canvas) {
  ...
  function draw(p) {
    ...
    grad.addColorStop(0, 'rgba(203,41,87,0.25)')
    grad.addColorStop(1, 'rgba(203,41,87,0.01)')
    ctx.fillStyle = grad
    ctx.fill()
    ctx.beginPath()
  ...
  if (hero) {
    ...
    hero.addEventListener('mousemove', (e) => {
      ...
      requestAnimationFrame(() => {
        ...
        glow.style.background = `radial-gradient(400px at ${cx}% ${cy}%, rgba(203,41,87,0.04) 0%, transparent 70%)`
